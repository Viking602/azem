  ...
  for (const block of blocks) {
    if (block.kind === "user") {
      for (const attachment of block.attachments ?? []) {
        imageIndex += 1;
        const title = imageSourceTitle(attachment.name, imageIndex, language);
        add({
          id: attachment.id || attachment.path || `image-${imageIndex}`,
          kind: "image",
          title,
          detail: attachment.name && attachment.name !== title ? attachment.name : undefined,
          attachment,
        });
      }
      for (const href of collectTextURLs(block.content || "")) {
        add({ id: `input:${href}`, kind: "input-url", title: urlDisplayTitle(href), detail: href, href });
      }
      continue;
    }
...
export function imageSourceTitle(name: string, index: number, language: Language): string {
  ...
  if (!stem || GENERIC_IMAGE.test(stem)) return language === "zh-CN" ? `图片 ${index}` : `Image ${index}`;
  if (PASTED_IMAGE.test(stem)) return language === "zh-CN" ? `粘贴图片 ${index}` : `Pasted image ${index}`;
  return trimmed;
}
...
function collectToolURLs(block: Block): Array<{ href: string; title: string }> {
  ...
  for (const text of texts) {
    for (const match of text.matchAll(MARKDOWN_LINK)) push(trimURLPunctuation(match[2] || ""), match[1] || "");
...
function collectJSONURLs(raw: string, push: (href: string, title?: string) => void) {
  for (const value of extractJSONValues(raw)) walkJSONForURLs(value, push);
}
