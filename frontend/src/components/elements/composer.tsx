...
export type ComposerContextTone = "primary" | "secondary" | "tertiary";

export interface ComposerContextSegment {
  key: string;
  label: string;
  tokens: number;
...
export interface ComposerUsage {
  segments: readonly ComposerContextSegment[];
  total: number;
}
...
export function ComposerTextarea({ className, ...props }: ComponentProps<"textarea">) {
  return <textarea
    data-slot="composer-input"
    className={cn(
      "placeholder:text-foreground/35 min-h-11 w-full resize-none bg-transparent px-3 py-2 text-[15px] leading-relaxed caret-blue-500 outline-none dark:caret-blue-400",
      className,
    )}
    {...props}
  />;
...
export function ComposerContext({
  ...
  const used = usage.segments.reduce((total, segment) => total + Math.max(0, segment.tokens), 0);
  const fraction = usage.total === 0 ? 0 : used / usage.total;
  const warn = fraction > 0.85;
  const circumference = 2 * Math.PI * 6;
  const toneClass = {
    primary: "bg-blue-500",
    secondary: "bg-amber-700/60",
    tertiary: "bg-foreground/35",
  } satisfies Record<ComposerContextTone, string>;
    ...
    >
      ...
      >
        <div className="flex items-baseline justify-between">
          ...
          <p className={cn(mono, "text-foreground/40 tabular-nums")}>
        ...
        <div className="bg-foreground/[0.06] flex h-[5px] w-full gap-px overflow-hidden rounded-full">
          {usage.segments.map((segment) => (
            <span
              ...
              style={{ width: `${pct(segment.tokens, usage.total)}%` }}
            />
          ))}
        </div>
        <div className="flex flex-col gap-2">
          {usage.segments.map((segment) => (
            ...
            >
              <span aria-hidden className={cn("size-1.5 rounded-full", toneClass[segment.tone])} />
              <span className="flex-1">{segment.label}</span>
              <span className={cn(mono, "text-foreground/35 tabular-nums")}>
                {Math.round(pct(segment.tokens, used))}%
              </span>
              <span className={cn(mono, "text-foreground/60 min-w-10 text-end tabular-nums")}>
        ...
        <div className="text-foreground/55 flex items-center justify-between text-[13px]">
          ...
          <span className={cn(mono, "text-foreground/40 tabular-nums")}>
            {formatContextTokens(used)} / {formatContextTokens(usage.total)}
          </span>
        </div>
      </div>
      <button
