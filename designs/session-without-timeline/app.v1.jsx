    ...
    <aside className="workbench" aria-label="工作现场">
      <div className="workbench-inner">
        ...
        <nav className="workbench-tabs" aria-label="工作现场分类">
          <button type="button" className={tab === "process" ? "active" : ""} onClick={() => setTab("process")}>过程</button>
          <button type="button" className={tab === "changes" ? "active" : ""} onClick={() => setTab("changes")}>变更</button>
          <button type="button" className={tab === "agents" ? "active" : ""} onClick={() => setTab("agents")}>子智能体</button>
          <button type="button" className={tab === "reasoning" ? "active" : ""} onClick={() => setTab("reasoning")}>推理</button>
        </nav>
    ...
    <React.Fragment>
      {steps.length ? (
        <section className="work-section">
          <header><strong>执行清单</strong><span>{scene.stepProgress || `${steps.filter((s) => s.state === "done").length} / ${steps.length}`}</span></header>
          <div className="step-list">
            {steps.map((step) => (
              <div key={step.label} className={`step ${step.state || ""}`}>
                <span className="step-mark">{step.state === "done" ? "✓" : ""}</span>
      ...
      {scene.judgement ? (
        <section className="work-section">
          <header><strong>当前判断</strong><span>commentary</span></header>
          <div className="thought-card"><strong>{scene.judgement.title}</strong>{scene.judgement.body}</div>
        </section>
      ) : null}
      <section className="work-section">
        <header><strong>工具活动</strong><span>非时间线</span></header>
        {tools.length === 0 ? (
          <div className="empty-hint">当前场景无工具事件</div>
        ) : tools.map((tool) => (
          <div key={tool.id} className="tool-row" data-state={tool.state}>
...
function ChangesPanel({ scene }) {
  ...
  return (
    <section className="work-section">
      <header><strong>文件变更</strong><span>{files.length ? `${files.length} 个文件` : "无"}</span></header>
      {files.length === 0 ? (
        <div className="empty-hint">排队中的写操作不进入此投影</div>
      ) : files.map((file) => (
      ...
      {scene.changeNote ? (
        <div className="thought-card" style={{ marginTop: 10 }}><strong>投影规则</strong>{scene.changeNote}</div>
      ) : null}
    </section>
  );
}
...
function AgentsPanel({ scene }) {
  ...
  return (
    <section className="work-section">
      <header><strong>子智能体</strong><span>{agents.length || 0} 个</span></header>
      {agents.length === 0 ? (
        <div className="empty-hint">无子智能体任务</div>
      ) : agents.map((agent) => (
        <button key={agent.id} type="button" className="agent-row agent-open" onClick={scene.onOpenAgent}>
          <span className="agent-face" style={agent.faceStyle || undefined}>{agent.face}</span>
          <span className="activity-copy"><strong>{agent.name}</strong><small>{agent.meta}</small></span>
        </button>
      ))}
    </section>
  );
...
function ReasoningPanel({ scene }) {
  return (
    <section className="work-section">
      <header><strong>推理轨迹</strong><span>thinking</span></header>
      {scene.reasoning ? (
        <details className="reasoning-fold" open={scene.running}>
          <summary>{scene.running ? "推理中" : "已折叠推理"} · {scene.reasoning.duration}</summary>
          <div className="reasoning-body">{scene.reasoning.body}</div>
        </details>
      ) : (
        <div className="empty-hint">本场景无 reasoning 块；thinking 永不伪装成 commentary</div>
      )}
    </section>
  );
}

function TaskBrief({ title, body, attachments, planMode }) {
  return (
    <header className="task-brief">
      <div className="task-brief-label">当前任务{planMode ? <em className="plan-badge">计划模式</em> : null}</div>
      <div>
        <h2>{title}</h2>
        {body ? <p>{body}</p> : null}
        {attachments?.length ? (
          <div className="attachment-row">
            {attachments.map((item) => (
              <span key={item} className="attachment-chip">📎 {item}</span>
            ))}
          </div>
        ) : null}
      </div>
...
function StatusStrip({ running, waiting, label, time, onStop }) {
  const state = waiting ? "waiting" : running ? "running" : "completed";
  return (
    <div className="doc-status" data-state={state}>
      <i></i>
      <strong>{label}</strong>
      <span>{waiting ? "等待模型首包" : running ? "正文与工作现场同步更新" : "结构保持不变 · 仅内容成熟"}</span>
      <span>{time}</span>
      {running || waiting ? (
...
function AnswerBody({ scene }) {
  ...
  return (
    <article className="answer-document">
      {(scene.answerSections || []).map((section) => (
        <section key={section.heading}>
          ...
          {section.decision ? (
            <div className="decision-block">
              {section.decision.map((item) => (
                <div key={item.title} className={`decision-item ${item.recommended ? "recommended" : ""}`}>
      ...
      {scene.streamingTail ? (
        <p className="streaming-tail">{scene.streamingTail}</p>
      ) : null}
      ...
      {scene.fileSummary ? (
        <div className="edited-summary">
          <header><strong>本轮改动</strong><span>{scene.fileSummary.count}</span></header>
          <div className="edited-list">
            {scene.fileSummary.files.map((f) => <code key={f}>{f}</code>)}
          </div>
        </div>
      ) : null}
    </article>
...
function buildScene(id, { forceRunning }) {
  ...
  if (id === "streaming") {
    return {
      ...common,
      taskTitle: "设计非 Timeline 的会话草稿",
      taskBody: "要覆盖工具、审批、子智能体、规划、失败等全部场景。",
      judgement: {
        title: "正在整理正文结构",
        body: "commentary 进入「当前判断」，成熟段落直接写入答案区，不创建独立时间线节点。",
      },
      reasoning: {
        duration: "18s",
      ...
      answerSections: [
        {
          ...
          paragraphs: [
            "主区域只保留任务、当前结论和最终交付。推理摘要、工具调用、文件变化与子智能体状态集中到右侧工作现场。",
            "运行中与完成后的页面结构一致：变化的是内容成熟度，不是页面形状。",
          ],
        },
      ],
      streamingTail: "接着补齐审批与规划中断卡的位置……",
      answerProgress: {
        title: "正在完成验证章节",
        percent: "62%",
        width: "62%",
      ...
      tools: [
        ...
        { id: "2", name: "grep", preview: "BlockKind", state: "completed", stateLabel: "完成" },
      ],
      steps: [
        { label: "读取会话展示约束", state: "done", time: "18s" },
        { label: "重组语义信息架构", state: "done", time: "34s" },
        { label: "生成非 Timeline 草稿", state: "active", time: "进行中" },
  ...
  if (id === "multiturn") {
    return {
      ...
      fileSummary: {
        count: "3 files",
        files: ["index.html", "app.jsx", "prototype.css"],
      },
      tools: [
        { id: "1", name: "read_file", preview: "Timeline.tsx", state: "completed", stateLabel: "完成" },
      ],
  ...
  return {
    ...
    judgement: {
      title: "方案已收敛",
      body: "保留工作文档作为默认模式；长任务可切换笔记本；短问答可用回合卡片。",
    },
    reasoning: {
      duration: "42s",
    ...
    steps: [
      { label: "读取会话展示与样式约束", state: "done", time: "18s" },
      { label: "重组语义信息架构", state: "done", time: "34s" },
      { label: "生成非 Timeline 草稿", state: "done", time: "1m12s" },
      { label: "浏览器检查与交付", state: "done", time: "26s" },
    ],
    stepProgress: "4 / 4",
    tools: [
      { id: "1", name: "read_file", preview: "Timeline.tsx", state: "completed", stateLabel: "完成" },
      { id: "2", name: "grep", preview: "BlockKind", state: "completed", stateLabel: "完成" },
      { id: "3", name: "write", preview: "designs/...", state: "completed", stateLabel: "完成" },
    ],
    files: [
      { icon: "H", path: "index.html", note: "页面入口", plus: "+28" },
      { icon: "C", path: "prototype.css", note: "非 Timeline 布局", plus: "+720" },
      { icon: "J", path: "app.jsx", note: "场景覆盖草稿", plus: "+900" },
    ],
    agents: [
      { id: "a1", face: "UX", name: "信息架构审查", meta: "已完成 · 3 条建议" },
      { id: "a2", face: "UI", name: "视觉一致性检查", meta: "已完成 · 无阻塞", faceStyle: { background: "#ece4f7", color: "#7553a5" } },
    ],
    answerSections: [
      {
        heading: "建议采用「工作文档」作为默认模式",
      ...
      {
        heading: "三个可选方向",
        paragraphs: ["同一事件源，三种阅读投影。"],
        decision: [
          { title: "工作文档", body: "正文稳定、过程旁置。适合作为默认会话模式。", recommended: true },
          { title: "回合卡片", body: "每个用户问题形成一张完整卡片，适合多轮短问答。" },
          { title: "会话笔记本", body: "按章节导航目标、决策、改动和验证，适合超长任务。" },
        ],
      },
      {
        heading: "关键交互规则",
        paragraphs: [],
        list: [
          "用户最新要求固定为任务 Brief，不再像气泡一样漂移",
          "运行中答案直接在正文持续成稿；不单独创建 Timeline 节点",
          "approval / question / plan / error 作为主栏中断卡",
          "工具状态机、文件变更、子智能体、thinking 进入工作现场",
          "子智能体抽屉与主会话同构；完成后过程默认折叠",
        ],
      },
    ],
    answerProgress: {
        ...
        <section className="session-stage">
          <header className="session-header">
            <div className="session-title">
              <span className="session-index">DRAFT v2</span>
              <div>
                <h1>{meta.title}</h1>
                <p>非 Timeline 会话 · 全场景草稿</p>
              </div>
            </div>
